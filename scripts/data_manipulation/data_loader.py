...
class DeepSDFDataset2D(Dataset):
    def __init__(self, root_folder, split="train", transform=None):
        ...
        label_dirs = sorted(
            [d for d in os.listdir(root_folder) if not d.startswith(".")]
        )
        ...
        for label in label_dirs:
            ...
            if not os.path.exists(label_csv_dir):
                ...
                continue
            csv_files = sorted(
                [f for f in os.listdir(label_csv_dir) if f.endswith(".csv")]
            )
            train_files, temp_files = train_test_split(
                csv_files, test_size=0.3, shuffle=True, random_state=42
            )
            if self.split == "train":
        ...
        self.sdf_values = [
            sdf_value for sdf_value in self.sdf_values if len(sdf_value) > 0
        ]
        self.shape_counts = [shape_count for shape_count in self.shape_counts]
        self.points = torch.tensor(
            np.concatenate(self.points, axis=0), dtype=torch.float32
        )
        self.sdf_values = torch.tensor(
            np.concatenate(self.sdf_values, axis=0), dtype=torch.float32
        ).unsqueeze(1)
        self.shape_counts = torch.tensor(
            np.concatenate(self.shape_counts, axis=0)
        ).unsqueeze(1)
