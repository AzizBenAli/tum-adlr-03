...
def visualize_shape_with_latent(model, latent_code, shape_count, grid_size=224, grid_range=(-10, 10), device='cpu'):
    ...
    with torch.no_grad():
        ...
        plt.tight_layout()
        plt.savefig(f"../plots/reconstructed_image_{shape_count}")
        plt.show()
...
def infer_and_visualize_shape(model, test_data_loader, shape_idx, grid_size=224, grid_range=(-10, 10), device='cpu', num_iterations=500, lr=1e-2):
    ...
    visualize_shape_with_latent(model, latent_code, batch['shape_count'], grid_size, grid_range, device)
...
if __name__ == "__main__":
    ...
    for i in range(0,5):
        infer_and_visualize_shape(trained_model, test_loader, i, grid_size=500, grid_range=(-448, 448), device=device,
                                  num_iterations=10000, lr=1e-1
                                  )
