import os, cv2, csv, trimesh, io, random
from utils import *

os.environ['PYOPENGL_PLATFORM'] = 'egl'
class MeshProcessor:
    def __init__(self, parent_directory, output_directory, resolution=(224, 224), rotation_angle=20, border_threshold=10.0,label=0):
    ...
    def process_mesh(self, mesh_path, shape_count, label):
        try:
            ...
            for i in range(sdf.shape[0]):
                for j in range(sdf.shape[1]):
                    if abs(sdf[i, j]) < self.border_threshold:  # Point near the border
                        ...
                        border_points.append([i, j, sdf[i, j], location, shape_count])
            ...
            filtered_points = [(i, j) for i in range(sdf.shape[0]) for j in range(sdf.shape[1]) if abs(sdf[i, j]) < 3]
            ...
            for i, j in random_points:
                ...
                final_points.append([i, j, sdf_value, location, shape_count])
            ...
            with open(csv_filename, mode="w", newline="") as file:
                ...
                writer.writerow(["x", "y", "sdf", "location", "shape_count", "label"])  # Add "label" column
                for point in final_points:
                    writer.writerow(point + [self.label])  # Add label to each row
            ...
            if "models" in dirs:
                ...
                for file_name in os.listdir(models_folder):
                    if file_name.endswith(".obj"):
                        ...
                        self.process_mesh(obj_file_path, shape_count, self.label)  # Pass label
                        shape_count += 1
                    if shape_count == 300:
                        break
...
if __name__ == "__main__":
    ...
    for i,shape in enumerate(shapes):
        ...
        processor = MeshProcessor(parent_directory=parent_directory, output_directory=output_dir,label=i)
