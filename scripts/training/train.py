...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Train SDF Loss: {avg_sdf_loss:.6f}, Train Latent Loss: {avg_latent_loss:.6f}"
        )
...
if __name__ == "__main__":
    ...
    torch.save(
        ...
        "../../multiclass/trained_models/trained_decoder/deepsdf_model.pth",
    )

    latent_codes = trained_model.get_all_latent_codes()
    os.makedirs("../../multiclass/trained_models/trained_latent_vector", exist_ok=True)
    np.save(
        "../../multiclass/trained_models/trained_latent_vector/latent_codes.npy",
        latent_codes.cpu().numpy(),
    )
