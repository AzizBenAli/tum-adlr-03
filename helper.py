...
def visualize_shape_with_latent_shape_completion(
    ...
    with torch.no_grad():
        ...
        if optimization_points is not None:
            ...
            shift = -grid_range[0]
            scaled_x = (optimized_points[:, 0] + shift) * (grid_size / (grid_range[1] - grid_range[0]))
            scaled_y = (optimized_points[:, 1] + shift) * (grid_size / (grid_range[1] - grid_range[0]))
            ...
            axes[2].scatter(
                scaled_y, -scaled_x,
                color='green', s=10, label='Optimization Points'
            )
            axes[2].legend()
